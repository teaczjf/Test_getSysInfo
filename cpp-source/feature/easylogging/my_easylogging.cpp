...
// #define LogInfoDump(info, size, buff) logDump(info, buff, size, LEVEL_LOG::MY_Info)
// #define LogTraceDump(info, size, buff) logDump(info, buff, size, LEVEL_LOG::MY_Trace)
// #define LogWarnDump(info, size, buff) logDump(info, buff, size, LEVEL_LOG::MY_Warn)
// #define LogErrorDump(info, size, buff) logDump(info, buff, size, LEVEL_LOG::MY_ERR)
// #define LogDebugDump(info, size, buff) logDump(info, buff, size, LEVEL_LOG::MY_Debug)
 ...
 * @param enable_log    1：开启后，调试信息打印进log文件  0：关闭
 * @param enable_terminal  1：开启后，调试信息打印终端上  0：关闭
 */
void easylogginginit ( int enable_log, int enable_terminal )
{
    ...
    */
    ...
    // const string log_conf_file = R"(./../../doc/log.conf)";
    const string log_conf_file = R"(./spec/log.conf)";
    // 加载配置文件，构造一个配置器对象
    ...
    En_G_LOG_DEBUG_INFO ( enable_log );       //开启后，调试信息打印进log文件
    En_G_DEBUG_TERMINAL_LOG ( enable_terminal ); //开启后，调试信息打印终端上
}
...
//            ss << static_cast<int>(buff[_dumpArrSize]) << " ";//这个是按照十进制的数打印
            // ss <<(buff[_dumpArrSize]) << " ";//这个是按照ASCII的数打印
        {
            ss << hex << static_cast<int> ( buff[_dumpArrSize] ) << " ";    //按照十六进行打印
        }
...
void logDump ( char *info, UINT8 *buff, int size, LEVEL_LOG logLevel )
{
    ...
    if ( buff == NULL || size <= 0 ) {
        ...
        {
            ss << std::hex << static_cast<u_int> ( buff[_dumpArrSize] ) << " ";
        }
        str += ss.str();
    }
    logStrOut ( size, str, logLevel );
